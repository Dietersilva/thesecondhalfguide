...
ABOUT_BODY = """  <div class="wrap">
    ...
    <div class="prose">
      ...
      <p>Here is how a correction actually works, so you know what to expect. Every figure is checked against the sources listed at the foot of that article before it is published, and those sources are named so you can check them yourself rather than take our word for it. Where a number cannot be confirmed, we leave it out instead of publishing our best guess &mdash; there are places on this site where a figure is deliberately missing for that reason.</p>
      <p>When something is wrong anyway, we fix the article itself rather than quietly editing around it, and we move the checked date only for what was genuinely re-verified. In August 2026 an outside fact-check went through every article here and found eighteen things to change, four of them real errors on figures that mattered. All eighteen were corrected. We would rather tell you that than imply it has never happened.</p>
